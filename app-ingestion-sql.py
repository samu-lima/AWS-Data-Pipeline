...
def createTables(cur):
    try:
        ...
        cur.execute("""
                    ...
                    """)
        print('Tables created successfully on database')
    except Exception as e:
        print(f'Error when trying to create tables on database: {e}')
    return 0

def randomData(cur, conn, numClients):
    try:
        #Insert clients on table Customers
        custormers = []
        for i in range(numClients):
            name = fake.name()
            email = fake.email()
            cur.execute("INSERT INTO Customers (name, email) VALUES (%s,%s)",(name, email))
            custormers.append(name)
        conn.commit()
        print('User data generetaded successfully')
    except Exception as e:
        print(f'Failed on trying to generate user data: {e}')

    try:
        #Insert products on table Products
        ...
        categories = ['Eletronics', 'Informatic', 'Cell', 'Accessories']
        ...
        print(f'Failed on trying to generate products data: {e}')    

    try:
        #Insert requests on table Orders
        for i in range(len(products)):
        ...
        conn.commit()
        print('Orders data generetaded successfully')
    except Exception as e:
        print(f'Failed on trying to generate requests data: {e}')

    #Close conection with the database
    cur.close()
    conn.close()
...
#create ambience variables for the file .env  
load_dotenv()
